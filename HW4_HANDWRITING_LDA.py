...
for i in range(10): #test with each class of data
    ...
    #C#onfusionTally_90 = np.zeros((1,10))
    #C#onfusionTally_95 = np.zeros((1,10))
...
plt.title("Confusion matrix for LDA Unreduced Data")
cfm_plot = sn.heatmap(dat_Confusion, annot=True)

'''
plt.figure(figsize = (10,7))
plt.title("Confusion matrix for LDA 90% Data")
cfm_plot = sn.heatmap(dat_Confusion_90, annot=True)

plt.figure(figsize = (10,7))
plt.title("Confusion matrix for LDA 95% Data")
...
plt.title("Comparison of Accuracy for LDA Classification with PCA Reduced Data")
